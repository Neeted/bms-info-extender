...
function createScoreViewerRenderer(canvas) {
  ...
  function render2(model, selectedTimeSec, pixelsPerSecond = DEFAULT_VIEWER_PIXELS_PER_SECOND) {
...
function createScoreViewerController({ root, onTimeChange = () => {
  ...
  function setSelectedTimeSec2(timeSec) {
    ...
    if (Math.abs(clampedTimeSec - state2.selectedTimeSec) < 5e-4 && state2.model) {
      syncScrollPosition();
      renderScene();
      return;
    }
  ...
  function syncTimeFromScrollPosition({ force = false } = {}) {
    ...
    const nextTimeSec = getTimeSecForScrollTop(state2.model, scrollHost.scrollTop, getPixelsPerSecond());
    if (Math.abs(nextTimeSec - state2.selectedTimeSec) < 5e-4) {
      return;
    }
  ...
  function refreshLayout() {
    ...
    spacer.style.height = `${getContentHeightPx(state2.model, height, getPixelsPerSecond())}px`;
    syncScrollPosition();
    renderScene();
  }
  function renderScene() {
    const cursor = getViewerCursor(state2.model, state2.selectedTimeSec);
    ...
    playbackButton.disabled = !state2.model;
    playbackButton.textContent = state2.isPlaying ? "❚❚" : "▶";
    playbackButton.setAttribute("aria-label", state2.isPlaying ? "Pause score viewer" : "Play score viewer");
    ...
    spacingValue.textContent = formatSpacingScale(state2.spacingScale);
    spacingInput.value = String(state2.spacingScale);
    const renderResult = renderer.render(showScene ? state2.model : null, cursor.timeSec, getPixelsPerSecond());
...
function createBmsInfoPreview({
  ...
  function setSelectedTimeSec2(nextTimeSec, { openViewer = false, notify = false } = {}) {
    const clampedTimeSec = clampSelectedTimeSec(state2, nextTimeSec);
    const changed = Math.abs(clampedTimeSec - state2.selectedTimeSec) >= 5e-4;
    if (openViewer) {
      state2.isViewerOpen = true;
    }
    state2.selectedTimeSec = clampedTimeSec;
  ...
  function startPlayback() {
    ...
    if (state2.selectedTimeSec >= maxTimeSec - 5e-4) {
      state2.selectedTimeSec = 0;
      onSelectedTimeChange(state2.selectedTimeSec);
    }
  ...
  function stepPlayback(timestamp) {
    ...
    const changed = Math.abs(nextTimeSec - state2.selectedTimeSec) >= 5e-4;
    state2.selectedTimeSec = nextTimeSec;
    if (changed) {
      onSelectedTimeChange(state2.selectedTimeSec);
    }
  ...
  function flushRender() {
    ...
    viewerController.setSelectedTimeSec(state2.selectedTimeSec);
...
function setSelectedTimeSec(nextValue, { openViewer = false, syncUrl = true } = {}) {
  ...
  const changed = Math.abs(normalizedValue - state.selectedTimeSec) >= 5e-4;
  if (openViewer) {
...
function ensurePreviewRuntime() {
  ...
  state.previewRuntime = createBmsInfoPreview({
    ...
    onSelectedTimeChange: (nextTimeSec) => {
      const changed = Math.abs(nextTimeSec - state.selectedTimeSec) >= 5e-4;
...
function renderPreviewPanel() {
  ...
  if (Math.abs(previewState.selectedTimeSec - state.selectedTimeSec) >= 5e-4 || shouldOpenViewer) {
